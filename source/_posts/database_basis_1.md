...
- 数据作为企业组织的公共资源而集中管理控制，能为企业各类用户共享，须大量地消除数据沉余，节省存储空间；
...
- 数据沉余小：将独立而又沉余的数据文件集成为单一的逻辑结构，每个数据项的值可以理想地只存储一次，从而节约空间，避免数据的重复存储。<label>并非所有的沉余都可以被消除，有时业务或技术上的原因，如数据的合法性检验、数据存储效率等方法，同一数据可能在数据库种保持多个副本，因此数据库系统种，沉余是受控的，系统知道沉余，保留必要的沉余是系统预定的</label>
- 数据一致性：通过消除或控制数据沉余，可在一定范围内避免数据的不一致性。
